...
def main():
    ...
    if not st.session_state['logged_in']:
        ...
        if st.button("접속하기", use_container_width=True):
            ...
            if not users_df.empty:
                match = users_df[(users_df['username'] == user_input.strip()) & 
                                 (users_df['password'] == make_hashes(pw_input))]
                if not match.empty:
        ...
        if menu == "⏰ 근태 관리":
            st.title("⏰ 근태 관리 (수동 갱신 버전)")
            # 에러 방지를 위해 자동 새로고침 대신 버튼으로 대체
            if st.button("🔄 시각 갱신"):
                st.rerun()
            ...
            if my_record.empty:
                if st.button("🚀 출근하기", use_container_width=True):
                    ...
                    conn.update(spreadsheet=SHEET_URL, worksheet="attendance", data=pd.concat([attendance_df, new_att]))
                    st.rerun()
            ...
            with st.form("allbaro_form"):
                ...
                st.metric("실중량", f"{gross - empty} kg")
                if st.form_submit_button("확정 저장"):
                    st.success("저장되었습니다.")

        elif menu == "📊 기록 조회":
            st.title("📊 업무 기록 조회")
            tab1, tab2 = st.tabs(["근태", "계량"])
            with tab1: st.dataframe(get_data("attendance"))
            with tab2: st.dataframe(get_data("reports"))
